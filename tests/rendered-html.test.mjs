...
test("renders the complete Astro Digital Solution landing page", async () => {
  ...
  assert.match(html, /Tiga kemampuan inti untuk membangun sistem yang tepat/);
  assert.match(html, /class="service-grid"/);
  assert.equal((html.match(/class="service-card(?: service-card-featured)?"/g) ?? []).length, 3);
  assert.match(html, /class="service-card service-card-featured"/);
  ...
  assert.match(html, /Aplikasi lapangan/);
  assert.match(html, /Asisten berbasis AI/);
  assert.doesNotMatch(html, /service-row|service-symbol|Selengkapnya/);
